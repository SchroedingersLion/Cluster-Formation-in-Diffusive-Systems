...
inline void simulation:: set_initial_position(const int seed){
    ...
    for (int i=0; i<model.N_particles; ++i){
        ...
        model.init_positions[i].z = box_uniform(twister);
    }
...
inline void simulation:: set_initial_velocities(){
    ...
    for (int i=0; i<model.N_particles; ++i){
        ...
        model.velocities[i].z = normal(twister);
    }
...
inline void simulation:: compute_force_par()
{
    ...
    for (auto& forces_for_specific_task : forces_for_all_tasks)
        std:: fill(forces_for_specific_task.begin(), forces_for_specific_task.end(), coordinate{0,0,0});
    ...
    for (int i = 0; i < model.N_particles; ++i) {
        ...
        forces_for_this_task[i].z += force_ij.z;
        forces_for_this_task[j].x += -force_ij.x;
        forces_for_this_task[j].y += -force_ij.y;
        forces_for_this_task[j].z += -force_ij.z;
        
        }
    }

    // Sum all of the task-specific forces into the output parameter.
    std:: fill(model.forces.begin(), model.forces.end(), coordinate{0, 0, 0});
    for (auto const& forces_for_specific_task : forces_for_all_tasks)
        ...
        model.forces[i].z += forces_for_specific_task[i].z;
        }
}
...
inline void simulation:: A_step(const double h){
    ...
    for (int i=0; i<model.N_particles; ++i){
        ...
        model.positions[i].z += h*model.velocities[i].z;
    }
...
inline void simulation:: B_step(const double h){
    ...
    for (int i=0; i<model.N_particles; ++i){
        ...
        model.velocities[i].z += h*model.forces[i].z;
    }
...
inline void simulation:: O_step(const double h){
    ...
    for (int i=0; i<model.N_particles; ++i){
        ...
        model.velocities[i].z = a*model.velocities[i].z + pref*normal(twister); 
    }
...
inline void simulation:: apply_periodic_boundaries(){
    ...
    for (int i=0; i<model.N_particles; ++i){
        ...
        z = model.positions[i].z;
        model.positions[i].z = z>L ? z-two_L : (z<-L ? z + two_L : z);
    }
}
